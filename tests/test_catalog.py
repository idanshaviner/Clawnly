...
def test_thirty_well_formed_activities_in_one_neighborhood():
    assert len(catalog.ACTIVITIES) == 30
    ids = set()
...
def test_get():
    assert catalog.get("sports-bar")["name"].startswith("Beers and the game")
